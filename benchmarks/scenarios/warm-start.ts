...
/**
 * Run a warm-start benchmark (mirrors already cached).
 * Measures fetch + worktree + install time.
 */
export async function runWarmStart(options: WarmStartOptions): Promise<BenchmarkTiming> {
  const { repos, cacheDir, workspaceDir } = options;

  // Only clean workspace, keep cache for warm start
  await cleanupBenchmarkDirs({ cacheDir, workspaceDir, keepCache: true });
  ...
  const currentRepoStartTime: Record<string, number> = {};

  timer.startPhase("git");

  for await (const state of stampWorkspaceGenerator({
    featureName: `benchmark-warm-${Date.now()}`,
    workspaceDir,
    repos,
  })) {
    switch (state.phase) {
