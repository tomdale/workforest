---
name: submit
description: Queue a finished Workforest feature branch for integration. Use when a feature branch is done and should enter the local integration flow.
---
...
3. Require a clean working tree and committed changes before enqueueing.
   If the checkout is dirty, delegate committing to the `commit` subagent before
   enqueueing.
...
3. If the worktree is dirty:
   ...
   - If the user approves or already requested commit-and-submit, delegate to the
     `commit` subagent using the `subagent` tool with `agentScope: "both"` (the agent
     lives in `.pi/agents/`). Include the worktree path, branch name, and intended
     scope.
